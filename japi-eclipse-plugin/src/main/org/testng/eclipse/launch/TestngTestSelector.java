...
public abstract class TestngTestSelector {
  ...
  /**
   * This method is final but non-private because it is called by the
   * constructor but also available to callers who wish to use the no-arg
   * constructor, construct their buttonHandler, and only then do this
   * initializing.
   * 
   * @param callback
   * @param buttonHandler
  ...
  final void init(TestNGMainTab callback, ButtonHandler buttonHandler,
    ...
    SelectionAdapter buttonAdapter = new ButtonAdapter(testngType,
        buttonHandler);
    Utils.Widgets wt = Utils.createWidgetTriple(comp, labelKey, radioAdapter,
        buttonAdapter, m_textAdapter);
