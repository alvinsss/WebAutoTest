...
/**
 * A rewriter that will convert the current JUnit file to TestNG using JDK5
 * annotations
 * 
...
public class AnnotationRewriter implements IRewriteProvider {
  ...
  public ASTRewrite createRewriter(CompilationUnit astRoot, AST ast) {
    ...
    //
    maybeAddImport(ast, result, astRoot, visitor.hasAsserts(),
    ...
    maybeAddImport(ast, result, astRoot, visitor.hasTestMethods(),
    ...
    for (String si : staticImports) {
      addImport(ast, result, astRoot, "org.testng.AssertJUnit." + si, true /*
                                                                            * static
                                                                            * import
                                                                            */);
    }
    ...
    maybeAddAnnotations(ast, visitor, result, visitor.getTestMethods(), "Test",
        null, null);
    maybeAddAnnotations(ast, visitor, result, visitor.getDisabledTestMethods(),
        "Test", null, createDisabledAttribute(ast));
    maybeAddAnnotations(ast, visitor, result, visitor.getBeforeMethods(),
        "BeforeMethod", "@Before" /* annotation to remove */);
    maybeAddAnnotations(ast, visitor, result, visitor.getAfterMethods(),
        "AfterMethod", "@After" /* annotation to remove */);

    //
    // suite() method: remove, comment out or leave untouched, depending on the
    // setting
    //
    SuiteMethodTreatment smt = TestNGPlugin.getPluginPreferenceStore()
        .getSuiteMethodTreatement();
    ...
    if (smt != SuiteMethodTreatment.DONT_TOUCH && suiteMethod != null) {
      if (smt == SuiteMethodTreatment.REMOVE) {
        ...
        ListRewrite lr = result.getListRewrite(type,
            TypeDeclaration.BODY_DECLARATIONS_PROPERTY);
        lr.insertBefore(
            result.createStringPlaceholder("/*", ASTNode.METHOD_DECLARATION),
            suiteMethod, null);
        lr.insertAfter(
            result.createStringPlaceholder("*/", ASTNode.METHOD_DECLARATION),
            suiteMethod, null);
    ...
    //
    for (Map.Entry<MethodDeclaration, Annotation> e : visitor
        .getIgnoredMethods().entrySet()) {
      ...
      ListRewrite lr = result.getListRewrite(md,
          MethodDeclaration.MODIFIERS2_PROPERTY);
      lr.insertFirst(test, null);
    }

    //
    // Replace "Assert" with "AssertJUnit", unless the method is already
    // imported statically.
    //
    ...
    //
    for (Map.Entry<MemberValuePair, String> pair : visitor
        .getTestsWithExpected().entrySet()) {
      result.replace(pair.getKey().getName(),
          ast.newSimpleName(pair.getValue()), null);
    }
    ...
    if (runWith != null) {
      ...
      // Add imports
      addImport(ast, result, astRoot,
          "org.testng.ConversionUtils.wrapDataProvider", true /* static import */);
      addImport(ast, result, astRoot, "org.testng.annotations.Factory", false /*
                                                                               * not
                                                                               * static
                                                                               */);
      ...
      md.setName(ast.newSimpleName("factory"
          + capitalize(parameterMethod.getName().toString())));
      ...
      md.modifiers()
          .addAll(ast.newModifiers(Modifier.PUBLIC | Modifier.STATIC));
      ArrayType returnType = ast.newArrayType(ast.newSimpleType(ast
          .newName("Object")));
      md.setReturnType2(returnType);

      // Create the method invocation
      // "ConversionUtils.wrapDataProvider(Foo.class, data())"
      ...
      tl.setType(ast.newSimpleType(ast.newSimpleName(visitor.getType()
          .getName().toString())));
      mi.arguments().add(tl);

      // 2) the call to the @Parameters method
      MethodInvocation pmi = ast.newMethodInvocation();
      pmi.setName(ast.newSimpleName(parameterMethod.getName()
  ...
  private void maybeAddImport(AST ast, ASTRewrite rewriter,
      CompilationUnit astRoot, boolean add, String imp) {
    if (add) {
      addImport(ast, rewriter, astRoot, imp);
    }
  }

  private void addImport(AST ast, ASTRewrite rewriter, CompilationUnit astRoot,
      String imp) {
    addImport(ast, rewriter, astRoot, imp, false /* non static import */);
  }

  private void addImport(AST ast, ASTRewrite rewriter, CompilationUnit astRoot,
      String imp, boolean isStatic) {
    ListRewrite lr = rewriter.getListRewrite(astRoot,
   ...
   */
  private void maybeAddAnnotation(AST ast, JUnitVisitor visitor,
      ASTRewrite rewriter, MethodDeclaration method, String annotation,
      String annotationToRemove, Map<String, Boolean> attributes) {
    if (method != null) {
      addAnnotation(ast, visitor, rewriter, method,
          createAnnotation(ast, annotation, attributes), annotationToRemove);
    }
  }
   ...
   */
  private Annotation createAnnotation(AST ast, String name,
  ...
  private void maybeAddAnnotations(AST ast, JUnitVisitor visitor,
      ASTRewrite rewriter, Collection<MethodDeclaration> methods,
      String annotation, String annotationToRemove) {
    maybeAddAnnotations(ast, visitor, rewriter, methods, annotation,
  ...
  private void maybeAddAnnotations(AST ast, JUnitVisitor visitor,
    ...
    for (MethodDeclaration method : methods) {
      maybeAddAnnotation(ast, visitor, rewriter, method, annotation,
          annotationToRemove, attributes);
    }
  }

  private void addAnnotation(AST ast, JUnitVisitor visitor,
      ASTRewrite rewriter, MethodDeclaration md, Annotation a,
      String annotationToRemove) {
    ListRewrite lr = rewriter.getListRewrite(md,
        MethodDeclaration.MODIFIERS2_PROPERTY);
    ...
    if (annotationToRemove != null) {
      ...
      for (int k = 0; k < modifiers.size(); k++) {
        ...
        if (old instanceof Annotation) {
          ...
          if (oldAnnotation.equals(annotationToRemove)
              || "@Override".equals(oldAnnotation)) {
