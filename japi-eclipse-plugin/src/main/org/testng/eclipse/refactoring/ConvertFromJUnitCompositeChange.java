...
public class ConvertFromJUnitCompositeChange extends CompositeChange {
  ...
  /**
   * Map of <source folder, Set<IResource>>. Note: using a set here since the
   * same resource can appear several times for a different IType (if the class
   * has nested classes) and we only want to perform the refactoring once per
   * file.
   */
