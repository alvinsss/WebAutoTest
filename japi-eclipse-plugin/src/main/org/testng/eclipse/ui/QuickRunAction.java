...
public class QuickRunAction extends Action {
  ...
  public QuickRunAction(IJavaProject javaProject, ILaunch prevLaunch,
      RunInfo runInfo, String mode) {
  ...
  private void initUI() {
    if (ILaunchManager.RUN_MODE.equals(m_runMode)) {
      ...
      setToolTipText(ResourceUtil
          .getString("QuickRunAction.run.action.tooltip")); //$NON-NLS-1$
      ...
      setToolTipText(ResourceUtil
          .getString("QuickRunAction.debug.action.tooltip")); //$NON-NLS-1$
      setDisabledImageDescriptor(TestNGPlugin
          .getImageDescriptor("dlcl16/debug.gif")); //$NON-NLS-1$
      setHoverImageDescriptor(TestNGPlugin
          .getImageDescriptor("elcl16/debug.gif")); //$NON-NLS-1$
  ...
  public void run() {
    ...
    if (null == imethod || m_previousRun == null)
      return;
    /*
     * The runInfo is passed along in order to preserve any jvm args used in the
     * original launcher when QuickRunAction is activated from the FailureTab to
     * re-run failed methods.
     */
    ...
    LaunchUtil.launchMethodConfiguration(m_javaProject, imethod, m_runMode,
        m_runInfo);
  }
