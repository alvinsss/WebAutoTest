...
public class TestNGPlugin extends AbstractUIPlugin implements ILaunchListener {
  ...
  public TestNGPlugin() {
    ...
    try {
      m_fgIconBaseURL = new URL(
          Platform.getBundle(PLUGIN_ID).getEntry("/"), "icons/full/"); //$NON-NLS-1$ //$NON-NLS-2$
  ...
  public void launchChanged(final ILaunch launch) {
    ...
    final int finalPort = isDebug() ? Integer.parseInt(RemoteTestNG.DEBUG_PORT)
        : port;
  ...
  public void connectTestRunner(ILaunch launch, IJavaProject launchedProject,
      String subName, int port) {
    IViewPart viewPart = showTestRunnerViewPartInActivePage(findTestRunnerViewPartInActivePage());
    // The returned view part could be an ErrorViewPart or null if something
    // went wrong in the init
    if (viewPart instanceof TestRunnerViewPart) {
      ((TestRunnerViewPart) viewPart).startTestRunListening(launchedProject,
          subName, port, launch);
    }
  }

  private IViewPart showTestRunnerViewPartInActivePage(
  ...
  private TestRunnerViewPart findTestRunnerViewPartInActivePage() {
    ...
    // If something went wrong, the JDT will return an ErrorViewPart, which
    // cannot
  ...
  /**
   * Make the passed label bold or not based on the boolean. This method is put
   * here because it initializes BOLD_FONT, which is later disposed in the stop
  ...
  public Bundle getBundle(String bundleName) {
    ...
    ServiceReference serviceRef = m_context
  ...
  /**
   * Returns a String that can be used as an identifying key for a system
   * property set when tests fail. The value of the property is the
   * comma-separated results, if any, of calling getTestName() on the failed
   * tests. This can be used by @Factory implementations to decide which
