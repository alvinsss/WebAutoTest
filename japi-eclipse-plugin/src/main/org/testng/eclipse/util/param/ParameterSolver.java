...
public class ParameterSolver {
  /**
   * TODO: this method searches only for parameters of the current
   * <code>javaElement</code> and not all the dependencies.
   */
  public static Map solveParameters(IJavaElement[] javaElements) {
    ...
    try {
      ...
      return findParameterValues(
          javaElements[0].getAncestor(IJavaElement.JAVA_PROJECT)
              .getCorrespondingResource(), paramNames);
  ...
  private static Map getParameterNames(IJavaElement javaElement)
  ...
  private static Map solveParameters(IPackageFragment packageFragment)
      throws JavaModelException {
    return parseParameterNames(packageFragment.getCompilationUnits(),
        new TestNGMethodParameterVisitor());
  }

  private static Map solveParameters(ICompilationUnit compilationUnit)
      throws JavaModelException {
    return parseParameterNames(new ICompilationUnit[] { compilationUnit },
        new TestNGMethodParameterVisitor());
  }

  private static Map solveParameters(IType type) {
    return parseParameterNames(
        new ICompilationUnit[] { type.getCompilationUnit() },
        new TestNGMethodParameterVisitor(type));
  }

  private static Map solveParameters(IMethod method) throws JavaModelException {
    if (method.getNumberOfParameters() > 0) {
      return parseParameterNames(
          new ICompilationUnit[] { method.getCompilationUnit() },
          new TestNGMethodParameterVisitor(method));
    }

    return null;
  }

  protected static Map parseParameterNames(ICompilationUnit[] units,
      TestNGMethodParameterVisitor visitor) {
  ...
  private static Map findParameterValues(IResource projectRes, Map parameters) {
    ...
    if (suiteFiles.length == 0) {
      ...
      // Otherwise, this will ultimately cause the test to fail with a clear
      // error.
      return new HashMap();
    }
    ...
    return selectedSuite != null ? extractParameterValues(selectedSuite,
  ...
  private static Map extractParameterValues(IFile file, Map parameters) {
    try {
      ...
      ParameterValuesContentHandler pvch = new ParameterValuesContentHandler(
          parameters);
  ...
  protected static IFile showSelectionDialog(IResource[] choices) {
    ...
    final SuiteListSelectionDialog dialog = new SuiteListSelectionDialog(
        parent, choices);
  ...
  protected static IResource[] searchSuites(IResource[] scopeResources) {
    ISearchQuery query = new FileSearchQuery(
        ...
        false /* regexp */, false /* casesensitive */,
        FileTextSearchScope.newSearchScope(scopeResources,
    ...
    public InputSource resolveEntity(String systemId, String publicId)
      ...
      if (Parser.DEPRECATED_TESTNG_DTD_URL.equals(publicId)
        ...
        if (null == is) {
          ...
          if (null == is) {
            System.out.println("WARNING: couldn't find in classpath "
