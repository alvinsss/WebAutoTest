...
public class TestNGMethodParameterVisitor extends ASTVisitor {
  ...
  private static final String PARAMETER_ANNOTATION_FQN = ANNOTATION_PACKAGE
      + PARAMETER_ANNOTATION;
  private static final String CONFIGURATION_ANNOTATION = "Configuration";
  private static final String CONFIGURATION_ANNOTATION_FQN = ANNOTATION_PACKAGE
      + CONFIGURATION_ANNOTATION;
  private static final String TEST_ANNOTATION = "Test";
  private static final String TEST_ANNOTATION_FQN = ANNOTATION_PACKAGE
      + TEST_ANNOTATION;
  ...
  public boolean visit(TypeDeclaration node) {
    if (null != m_typeFilter) {
      return node.getName().getIdentifier()
          .equals(m_typeFilter.getElementName());
    } else {
      return true;
    }
  }
  ...
  public boolean visit(MethodDeclaration node) {
    if (null != m_methodFilter) {
      return node.getName().getIdentifier()
          .equals(m_methodFilter.getElementName())
      ...
      for (int i = 0; i < values.size(); i++) {
        ...
        if ("parameters".equals(pair.getName().toString())) {
          ...
          if (paramAttr instanceof ArrayInitializer) {
            record((MethodDeclaration) annotation.getParent(),
                (ArrayInitializer) paramAttr);
          } else if (paramAttr instanceof StringLiteral) {
            record((MethodDeclaration) annotation.getParent(),
                (StringLiteral) paramAttr);
          }
        }
      }
    }
  ...
  public boolean visit(SingleMemberAnnotation annotation) {
    if (isParameterAnnotation(annotation.getTypeName().getFullyQualifiedName())) {
      ...
      if (paramValues instanceof ArrayInitializer) {
        record((MethodDeclaration) annotation.getParent(),
            (ArrayInitializer) paramValues);
      } else if (paramValues instanceof StringLiteral) {
        record((MethodDeclaration) annotation.getParent(),
            (StringLiteral) paramValues);
      }
    }
   ...
   * 
   * @param method
   *          the known TestNG annotation with required parameters
   * @param values
   *          array initializer containing StringLiterals for the parameter
   *          names
   */
  ...
  protected boolean isParameterAnnotation(String annotationType) {
    return PARAMETER_ANNOTATION.equals(annotationType)
        || PARAMETER_ANNOTATION_FQN.equals(annotationType);
  }
