...
public class PreferenceStoreUtil {
  ...
  public void storeOutputDir(String projectName, String outdir,
    ...
    m_storage.setValue(projectName + TestNGPluginConstants.S_ABSOLUTEPATH,
        isAbsolute);
  }
  ...
  public void storeUseProjectJar(String projectName, boolean selection) {
    m_storage.setValue(projectName + TestNGPluginConstants.S_USEPROJECTJAR,
        selection);
  }

  public void storeXmlTemplateFile(String projectName, String xmlFile) {
    m_storage.setValue(projectName + TestNGPluginConstants.S_XML_TEMPLATE_FILE,
        xmlFile);
  }

  public void storePreDefinedListeners(String projectName, String listeners) {
    m_storage.setValue(projectName
        + TestNGPluginConstants.S_PRE_DEFINED_LISTENERS, listeners);
  }

  public String getExcludedStackTraces(String projectName) {
    return getString(projectName, false,
  ...
  public String getPreDefinedListeners(String projectName, boolean projectOnly) {
    return getString(projectName, projectOnly,
        TestNGPluginConstants.S_PRE_DEFINED_LISTENERS);
  }
  ...
  public IPath getOutputDirectoryPath(IJavaProject project) {
    ...
    return new Path(isAbsolute ? outdir : project.getPath().toOSString() + "/"
        + outdir);
  }
  ...
  public IPath getOutputAbsolutePath(IJavaProject project) {
    ...
    return new Path(isAbsolute ? outdir : project.getProject().getLocation()
        .toOSString()
        + "/" + outdir);
  }
  ...
  public String getOutputDir(String projectName, boolean projectOnly) {
    String result = getString(projectName, projectOnly,
        TestNGPluginConstants.S_OUTDIR);
  ...
  public boolean isOutputAbsolutePath(String projectName, boolean projectOnly) {
    if (projectOnly
        || m_storage.contains(projectName
            + TestNGPluginConstants.S_ABSOLUTEPATH)) {
    ...
    if (m_storage.contains(TestNGPluginConstants.S_DEPRECATED_ABSOLUTEPATH)) {
      m_storage
          .setValue(TestNGPluginConstants.S_DEPRECATED_ABSOLUTEPATH, false);
      m_storage
          .setValue(TestNGPluginConstants.S_ABSOLUTEPATH, m_storage
              .getBoolean(TestNGPluginConstants.S_DEPRECATED_ABSOLUTEPATH));
    }
  ...
  public static enum SuiteMethodTreatment {
    REMOVE("Remove"), COMMENT_OUT("Comment out"), DONT_TOUCH("Don't touch");
