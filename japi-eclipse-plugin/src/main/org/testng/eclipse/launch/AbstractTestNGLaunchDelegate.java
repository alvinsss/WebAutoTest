 ...
 */
public abstract class AbstractTestNGLaunchDelegate implements
    ...
    if (editorInput instanceof IFileEditorInput) {
      ...
      if ("java".equals(file.getFileExtension())) {
        ...
        if (testContent.isTestNGClass()) {
          ...
          m_launchAttributes = LaunchUtil.createClassLaunchConfigurationMap(
              mainType, types, testContent.getAnnotationType());
        }
      }
      ...
      // m_launchAttributes.put(TestNGLaunchConfigurationConstants.TYPE,
      // SUITE_TYPE);
  ...
  public void run(IAction action) {
    ...
    LaunchUtil.launchMapConfiguration(m_project, m_configName,
        m_launchAttributes, m_compilationUnit, getLaunchMode());
  }
  ...
  /*
   * protected IType[] getTypes(IFile file) { try { ICompilationUnit
   * compilationUnit= JDTUtil.getJavaElement(file); return
   * compilationUnit.getTypes(); } catch(JavaModelException jme) {
   * TestNGPlugin.log(jme); }
   * 
   * return null; }
   */
