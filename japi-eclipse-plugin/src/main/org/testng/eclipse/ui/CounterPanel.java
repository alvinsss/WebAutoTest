...
/**
 * A panel presenting report info: 1st row: suite counters, test counters,
 * method counters 2nd row: passed, failed, skipped and successPercentageFailed
 * counters
 * <P/>
...
public class CounterPanel extends Composite {
  ...
  private final Image m_successIcon = TestNGPlugin.getImageDescriptor(
      "ovr16/success.gif").createImage(); //$NON-NLS-1$
  private final Image m_failureIcon = TestNGPlugin.getImageDescriptor(
      "ovr16/failure.gif").createImage(); //$NON-NLS-1$
  private final Image m_skipIcon = TestNGPlugin.getImageDescriptor(
      "ovr16/skip.gif").createImage(); //$NON-NLS-1$
  ...
  private void createReportUpperRow() {
    ...
    // "0/0          ", backgroundColor);
    // m_testCountText = createLabel(upperRow, "Tests:", null, "0/0         ",
    // backgroundColor);
  ...
  private Text createReportFor(Composite parent, String labelText,
      Color backColor, String initialText) {
  ...
  /*
   * private void createReportLowerRow() { Composite lowerRow = new
   * Composite(this, SWT.NONE); GridLayout gridLayout = new GridLayout();
   * gridLayout.numColumns = 3; gridLayout.makeColumnsEqualWidth = false;
   * gridLayout.marginWidth = 0; gridLayout.verticalSpacing= 0;
   * gridLayout.horizontalSpacing= 5; lowerRow.setLayoutData(new
   * GridData(GridData.FILL_HORIZONTAL)); lowerRow.setLayout(gridLayout);
   * 
   * Color backgroundColor =
   * getDisplay().getSystemColor(SWT.COLOR_WIDGET_BACKGROUND);
   * 
   * // m_passedText = createLabel(lowerRow, "Passed: 0", m_successIcon, "0",
   * backgroundColor); //$NON-NLS-1$ //$NON-NLS-2$ // m_failedText =
   * createLabel(lowerRow, "Failed: 0", m_failureIcon, "0", backgroundColor);
   * //$NON-NLS-1$ //$NON-NLS-2$ // m_skippedText = createLabel(lowerRow,
   * "Skipped: 0", m_skipIcon, "0", backgroundColor); //$NON-NLS-1$
   * //$NON-NLS-2$ // m_successPercentageFailedText = createLabel(lowerRow, //
   * "SPF:", //$NON-NLS-1$ // m_failPercentIcon, // " 0 ", //$NON-NLS-1$ //
   * backgroundColor); }
   */

  private CLabel createLabel(Composite parent, String name, Image image,
      String init, Color backColor) {
    // Label label = null;
    // if(image != null) {
