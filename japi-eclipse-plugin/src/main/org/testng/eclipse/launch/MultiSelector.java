...
/**
 * Abstract base class for launch configurations that allow the user to select
 * one or more strings from a dialog.
 * 
 * @author cbeust
 */
abstract public class MultiSelector extends TestngTestSelector {
  ...
  protected MultiSelector(TestNGMainTab callback, Composite comp,
      LaunchType type, String labelKey, String titleId) {
    super();
   ...
   * 
   * @param titleId
   *          the id for the title of the dialog.
   */
  public void handleMultiSearchButtonSelected(String titleId) {
    ...
    if (values.size() > 0) {
      ...
      final CheckBoxTable cbt = getCheckBoxTable(getCallback().getShell(),
          uniqueValues, titleId);
