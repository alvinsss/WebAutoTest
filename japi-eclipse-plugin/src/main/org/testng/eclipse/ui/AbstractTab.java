...
abstract public class AbstractTab extends TestRunTab implements IMenuListener {
  ...
  public Composite createTabControl(Composite parent, TestRunnerViewPart runner) {
    ...
    m_failureTraceComponent = new FailureTrace(stackTraceForm,
        m_testRunnerPart, failureToolBar);
  ...
  void handleDoubleClick(MouseEvent e) {
    ...
    if (testInfo instanceof TestMethodTreeItem) {
      OpenTestAction action = new OpenTestAction(m_testRunnerPart,
          testInfo.getRunInfo());
  ...
  /**
   * @return the RunInfo associated with the current selection, or null it not
   *         applicable.
   */
  private ITreeItem getSelectedTreeItem() {
    TreeItem[] treeItems = m_tree.getSelection();

    return treeItems.length == 0 ? null : BaseTreeItem
        .getTreeItem(treeItems[0]);
  }
  ...
  public void menuAboutToShow(IMenuManager manager) {
    if (m_tree.getSelectionCount() > 0) {
      ...
      manager.add(new QuickRunAction(m_testRunnerPart.getLaunchedProject(),
          m_testRunnerPart.getLastLaunch(), testInfo, ILaunchManager.RUN_MODE));
      manager
          .add(new QuickRunAction(m_testRunnerPart.getLaunchedProject(),
              m_testRunnerPart.getLastLaunch(), testInfo,
  ...
  /**
   * This method is invoked whenever a new test result arrives. Before adding
   * the corresponding method node, it makes sure that all its ancestors (class,
   * test, suite) exist and create them if they don't. Then it adds the time of
   * the new test method to all these nodes.
   */
  @Override
  ...
  public void restoreState(IMemento memento) {
    ...
    if (ratio != null) {
      m_sashForm.setWeights(new int[] { ratio.intValue(),
          1000 - ratio.intValue() });
    }
