...
/**
 * A view that shows a summary of the test run and allows the user to sort these
 * results by timings, names, counts, etc...
 */
public class SummaryTab extends TestRunTab {
  ...
  private void createExcludedMethodViewer(Composite result) {
    ...
    m_excludedMethodViewer = createViewer(result, new String[] { "Class name",
        "Method name", "Description" }, new int[] { 300, 250, 300 },
        new StringTableSorter(this), m_excludedMethodFilter);
    ...
    m_excludedMethodViewer.getTable().addMouseListener(new MouseAdapter() {
      ...
      private void handleDoubleClick(MouseEvent e) {
        ...
        if (items.length > 0) {
          ...
          System.out.println("Double click " + em.packageName + "."
              + em.methodName);
          OpenTestAction openAction = new OpenTestAction(m_testRunnerPart,
              em.packageName, em.methodName, true /* activate */);
          openAction.run();
        }
      }
    });
  ...
  private void createTestViewer(Composite result) {
    ...
    m_testViewer = createViewer(result, new String[] { "Test name",
        "Time (seconds)", "Class count", "Method count" }, new int[] { 150,
        150, 100, 100 }, new RunInfoTableSorter(this), m_testSearchFilter);
      ...
      public String getColumnText(Object element, int columnIndex) {
        ...
        case 1:
          return MessageFormat.format("{0}",
              ((float) getTestTime(testId)) / 1000);
        case 2:
          return Integer.toString(getTestClassCount(testId));
        case 3:
          return Integer.toString(getTestMethodCount(testId));
        default:
          return "";
        }
      }
  ...
  private TableViewer createViewer(Composite parent, String[] columns,
      int[] bounds, final AbstractTableSorter tableSorter, ViewerFilter filter) {
