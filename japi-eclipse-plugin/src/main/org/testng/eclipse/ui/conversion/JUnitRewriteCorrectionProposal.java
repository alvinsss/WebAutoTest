...
/**
 * This class implements the proposal to convert a JUnit class into a TestNG
 * one.
 * 
...
public class JUnitRewriteCorrectionProposal implements IJavaCompletionProposal {
  ...
  private String getHtml() {
    ...
    try {
      ...
      final IDocument previewContent = change
          .getPreviewDocument(new NullProgressMonitor());
      ...
      class EditAnnotator extends TextEditVisitor {
        ...
        private boolean rangeAdded(TextEdit edit) {
          ...
          appendContent(previewContent, edit.getOffset(),
              edit.getExclusiveEnd(), buf, false);
          buf.append("</b>"); //$NON-NLS-1$
  ...
  private void appendContent(IDocument text, int startOffset, int endOffset,
    ...
    try {
      ...
      if (surroundLinesOnly && startOffset == 0) { // no surround lines for the
                                                   // top no-change range
      ...
      for (int i = startLine; i <= endLine; i++) {
        if (surroundLinesOnly) {
          if ((i - startLine > m_surroundLines)
              && (endLine - i > m_surroundLines)) {
            if (!dotsAdded) {
              buf.append("...<br>"); //$NON-NLS-1$
              dotsAdded = true;
        ...
        if (surroundLinesOnly && (from == start)
            && containsOnlyWhitespaces(content)) {
          continue; // ignore empty lines except when range started in the
                    // middle of a line
        }
