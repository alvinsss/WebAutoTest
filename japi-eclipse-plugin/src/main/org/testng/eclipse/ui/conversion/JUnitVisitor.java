...
/**
 * This visitor stores all the interesting things in a JUnit class: - JUnit
 * imports - "extends TestCase" declaration - Methods that start with test -
 * setUp and tearDown - ...
 * 
...
public class JUnitVisitor extends Visitor {
  ...
  // The position and length of all the Assert references that are not
  // statically
  // imported
  ...
  // True if there are test methods (if they are annotated with @Test, they
  // won't
  ...
  private Map<MethodDeclaration, Annotation> m_ignoredMethods = Maps
      .newHashMap();
  ...
  public boolean visit(SuperConstructorInvocation sci) {
    ...
    if (m_testCase != null) {
      ...
      if (args.size() == 1) {
        ...
        if (binding != null
            && String.class.getName().equals(binding.getBinaryName())) {
  ...
  public boolean visit(MethodDeclaration md) {
    ...
    if (methodName.equals("setUp") || hasAnnotation(md, "Before")) {
      ...
      // Methods that start with "_test" or private test methods that start with
      // "test" are disabled
      boolean isPrivate = (md.getModifiers() & Modifier.PRIVATE) != 0;
      if (methodName.startsWith("test") && !isPrivate) {
        m_testMethods.add(md);
      } else if (methodName.startsWith("_test")
          || (methodName.startsWith("test") && isPrivate)) {
        m_disabledTestMethods.add(md);
      }
    } else if (hasAnnotation(md, "Test")) {
      m_hasTestMethods = true; // to make sure we import
  ...
  /**
   * Find occurrences of "Assert.xxx", which need to be replaced with
   * "AssertJUnit.xxx".
   */
  @Override
  ...
  private boolean belongsToAssertJUnit(MethodInvocation method) {
    ...
    // emulate type conversions (for example, (int, long) should become (long,
    // long)

    // Try to find a method with the exact signature. This can fail for a few
    // reasons
    // (such as not being able to resolve the binary name), in which case I
    // should try
    // to fall back on a simple name search
    try {
      Object m = AssertJUnit.class.getMethod(method.getName()
          .getFullyQualifiedName(), types.toArray(new Class[types.size()]));
      result = true;
    ...
    if (!result && (arguments.size() == 2 || arguments.size() == 3)) {
      // An assert with two or three parameters will match assertTrue(Object,
      // Object)
      result = true;
    }
    return result;
   ...
   * resolve the type conversion. For example, an invocation of assert(..., int,
   * long) will probably end up calling assert(..., long, long).
   */
  private void adjustForOverloading(List<Class> types) {
    if (types.size() > 2) {
      Class t2 = types.get(1);
      Class t3 = types.get(2);
      if ((t2 == long.class && t3 == int.class)
          || (t3 == long.class && t2 == int.class)) {
  ...
  /**
   * All the @Test annotated methods that have attributes that need to be
   * replaced.
   */
  ...
  /**
   * @return whether this file needs to be changed in order to be converted to
   *         TestNG.
   */
  public boolean needsConversion() {
    ...
    if (m_hasTestMethods || getTestMethods().size() > 0
        || getDisabledTestMethods().size() > 0 || getBeforeMethods().size() > 0
        || getAfterMethods().size() > 0 || m_suite != null) {
      return true;
    }
