...
public class TestNGMainTab extends AbstractLaunchConfigurationTab implements
  ...
  private void createSelectors(Composite comp) {
    ...
    m_classSelector = new TestngTestSelector(this, handler, LaunchType.CLASS,
      ...
      public void initializeFrom(ILaunchConfiguration configuration) {
        List<String> testClassNames = ConfigurationHelper
    ...
    m_methodSelector = new TestngTestSelector(this, handler, LaunchType.METHOD,
        comp, "TestNGMainTab.label.method") {
      @Override
      public void initializeFrom(ILaunchConfiguration configuration) {
    ...
    // Work in progress
    m_suiteSelector = new SuiteSelector(this, comp);
  ...
  public void performApply(ILaunchConfigurationWorkingCopy configuration) {
    ConfigurationHelper.updateLaunchConfiguration(
        ...
        new ConfigurationHelper.LaunchInfo(m_projectText.getText(),
            m_typeOfTestRun, StringUtils.stringToList(m_classSelector.getText()
                .trim()), StringUtils.stringToList(m_packageSelector.getText()
                .trim()), m_classMethods, m_groupSelector.getValueMap(),
   ...
   * 
   * @param testngType
   *          - one of TestNGLaunchConfigurationConstants
   */
  void handleSearchButtonSelected(LaunchType testngType) {
    ...
    if (selectedProject == null) {
      MessageDialog.openError(PlatformUI.getWorkbench()
          .getActiveWorkbenchWindow().getShell(), "No project",
    ...
    try {
      ...
      case CLASS:
        ...
        dialog = TestSelectionDialog.createTestTypeSelectionDialog(getShell(),
            m_selectedProject, types, Filters.SINGLE_TEST);
        break;
      case METHOD:
        types = TestSearchEngine.findMethods(getLaunchConfigurationDialog(),
            new Object[] { m_selectedProject }, m_classSelector.getText());
        dialog = TestSelectionDialog.createMethodSelectionDialog(getShell(),
    ...
    if (type != null) {
      ...
      case CLASS:
        m_classSelector
            .setText((((IType) type).getFullyQualifiedName()).trim());
      ...
      case SUITE:
        ...
        m_suiteSelector.setText(file.getProjectRelativePath().toOSString()
            .trim());
        break;
      case PACKAGE:
  ...
  void setType(LaunchType type) {
    if (type != m_typeOfTestRun) {
      ...
      // ////m_classMethods = null; // we reset it here, because the user has
      // changed settings on front page
      for (TestngTestSelector sel : m_launchSelectors) {
