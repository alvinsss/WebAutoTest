...
public class SuiteBuilder {
  ...
  public static File createSuite(IJavaProject ijp, IType[] types,
      IJavaElement ije) {
  ...
  // 1,
  // "Unknown annotation type '" + annotationType + "' Using default: " +
  ...
  private static void createClassTest(IType[] types, IJavaElement ije,
      XMLStringBuffer buf) {
    ...
    for (int i = 0; i < types.length; i++) {
      ...
      if (content.hasTestMethods()) {
        // attrs.setProperty("annotations",
        // convert(content.getAnnotationType()));
  ...
  private static void createMethodTest(IType[] types, IJavaElement ije,
      XMLStringBuffer buf) {
  ...
  private static final void saveFileContent(final File file,
      final XMLStringBuffer xmlBuffer) {
    FileWriter fw = null;
