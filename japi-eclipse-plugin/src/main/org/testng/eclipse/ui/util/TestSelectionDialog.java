...
public class TestSelectionDialog extends TwoPaneElementSelector {
  ...
  public static TestSelectionDialog createSuiteSelectionDialog(
      final Shell shell, final IJavaProject jproject, final Object[] resources) {
    TestSelectionDialog result = new TestSelectionDialog(shell,
        LaunchType.SUITE, jproject, new FileLabelProvider(
            FileLabelProvider.SHOW_LABEL), new FileLabelProvider(
            FileLabelProvider.SHOW_LABEL_PATH), resources, null);
  ...
  private static TestSelectionDialog createJavaElementDialog(final Shell shell,
      ...
      final LaunchType testngType, final String title,
      final Filters.ITypeFilter filter) {
    TestSelectionDialog result = new TestSelectionDialog(shell, testngType,
        jproject, new JavaElementLabelProvider(
  ...
  public int open() {
    if (null == m_input) {
      ...
      case CLASS:
        ...
        try {
          m_input = TestSearchEngine.findTests(new Object[] { m_project },
              m_filter);
      ...
      case PACKAGE:
        ...
        try {
          m_input = TestSearchEngine.findPackages(new Object[] { m_project });
        } catch (InterruptedException e) {
          return CANCEL;
        } catch (InvocationTargetException e) {
          TestNGPlugin.log(e.getTargetException());
        ...
        break;
      default:
        throw new IllegalArgumentException("testng type not yet implemented: "
            + m_testngType);
      }
