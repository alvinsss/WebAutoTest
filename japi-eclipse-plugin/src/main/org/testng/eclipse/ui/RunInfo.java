...
public class RunInfo {
  ...
  // this(trm.getSuiteName(), trm.getName(), trm.getTestClass(),
  // trm.getMethod(),
  ...
  public RunInfo(String suiteName, String testName, String className,
      String methodName, String testDesc, String instanceName, String[] params,
      String[] paramTypes, long time, String stackTrace, int status,
      int invocationCount, int currentInvocationCount) {
    m_id = suiteName + "." + testName + "." + className + "." + methodName
        + toString(params, paramTypes);
    if (testDesc != null)
      m_idWithDesc = m_id + "." + testDesc;
    else
      m_idWithDesc = m_id;
    m_suiteName = suiteName;
    m_testName = testName;
    ...
    m_testDescription = testDesc != null ? (testDesc.equals(methodName) ? null
        : testDesc) : null;
  ...
  private String toString(String[] params, String[] paramTypes) {
    ...
    for (int i = 0; i < params.length; i++) {
      if (i > 0)
        buf.append(",");
      if ("java.lang.String".equals(paramTypes[i])
          && !("null".equals(params[i]) || "\"\"".equals(params[i]))) {
  ...
  /*
   * String escapeNewLines(String s) { if(s.indexOf('\n') != -1 ||
   * s.indexOf('\r') != -1) { return s.replace("\n", "\\n").replace("\r",
   * "\\r"); }
   * 
   * return s; }
   */
  ...
  /**
   * Returns <code>true</code> if this <code>RunInfo</code> is the same as the o
   * argument.
   * 
   * @return <code>true</code> if this <code>RunInfo</code> is the same as the o
   *         argument.
   */
  @Override
  ...
  public String getTestDescription() {
    if (null == m_testDescription || "".equals(m_testDescription.trim()))
      return "";
    else
  ...
  public String getTreeLabel() {
    return getMethodName() + getParametersDisplay() + "("
  ...
  /*
   * public static void main(String[] args) { String test1= "something\nwrong";
   * String test2= "something\rwrong"; String test3=
   * "something\ndefinitely\rwrong"; String test4= "something\\n not\\r wrong";
   * String test5= "something not wrong\n"; String test6=
   * "something not wrong\n\r"; String test7= "something not wrong\r\n";
   * 
   * RunInfo info= new RunInfo("doesntmatter"); System.out.println("1: " +
   * info.escapeNewLines(test1)); System.out.println("2: " +
   * info.escapeNewLines(test2)); System.out.println("3: " +
   * info.escapeNewLines(test3)); System.out.println("4: " +
   * info.escapeNewLines(test4)); System.out.println("5: " +
   * info.escapeNewLines(test5)); System.out.println("6: " +
   * info.escapeNewLines(test6)); System.out.println("7: " +
   * info.escapeNewLines(test7)); System.out.println("1: " +
   * info.escapeNewLines2(test1)); System.out.println("2: " +
   * info.escapeNewLines2(test2)); System.out.println("3: " +
   * info.escapeNewLines2(test3)); System.out.println("4: " +
   * info.escapeNewLines2(test4)); System.out.println("5: " +
   * info.escapeNewLines2(test5)); System.out.println("6: " +
   * info.escapeNewLines2(test6)); System.out.println("7: " +
   * info.escapeNewLines2(test7)); }
   */
}
