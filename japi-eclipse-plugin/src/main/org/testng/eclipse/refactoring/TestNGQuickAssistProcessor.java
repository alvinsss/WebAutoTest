 ...
 */
public class TestNGQuickAssistProcessor extends BaseQuickAssistProcessor
  ...
  public IJavaCompletionProposal[] getAssists(IInvocationContext context,
    ...
    if (hasAssists(context)) {
      ...
      if (hasPushAssists(m_visitor))
        providers.add(new PushTestRewriter());
      if (hasPullAssists(m_visitor))
      ...
      for (IRewriteProvider provider : providers) {
        ...
        vResult.add(new JUnitRewriteCorrectionProposal(provider.getName(),
            m_compilationUnit, rewriter, 1));
      }
    }
