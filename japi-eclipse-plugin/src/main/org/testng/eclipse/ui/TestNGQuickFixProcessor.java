...
public class TestNGQuickFixProcessor implements IQuickFixProcessor {
  ...
  public IJavaCompletionProposal[] getCorrections(IInvocationContext context,
    ...
    if (!isJava5SyntaxSupported(project)
        || isTestNGContainerOnClasspath(project)) {
      return null;
    }
  ...
  private boolean isTestNGContainerOnClasspath(IJavaProject project) {
    try {
      return BuildPathSupport.projectContainsClasspathEntry(project,
          BuildPathSupport.getTestNGClasspathEntry());
  ...
  private List getAddTestNGToBuildPathProposals(IInvocationContext context,
      IProblemLocation location, List proposals) {
    try {
      ICompilationUnit compilationUnit = context.getCompilationUnit();
      IJavaProject project = compilationUnit.getJavaProject();
      String s = compilationUnit.getBuffer().getText(location.getOffset(),
          location.getLength());
  ...
  private boolean maybeTestNGAnnotation(String s) {
    return "Test".equals(s) || "Configuration".equals(s)
        || "BeforeSuite".equals(s) || "AfterSuite".equals(s)
        || "BeforeTest".equals(s) || "AfterTest".equals(s)
        || "BeforeGroups".equals(s) || "AfterGroups".equals(s)
        || "BeforeClass".equals(s) || "AfterClass".equals(s)
