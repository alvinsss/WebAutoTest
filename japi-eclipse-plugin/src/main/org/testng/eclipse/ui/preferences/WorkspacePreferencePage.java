...
public class WorkspacePreferencePage extends FieldEditorPreferencePage
  ...
  public void createFieldEditors() {
    ...
    m_outputdir = new FSBrowseDirectoryFieldEditor(
        TestNGPluginConstants.S_OUTDIR, "Output directory:", //$NON-NLS-1$ 
        parentComposite);
    ...
    btn.addSelectionListener(new SelectionAdapter() {
      ...
      public void widgetSelected(SelectionEvent evt) {
        m_absolutePath.getChangeControl(getFieldEditorParent()).setSelection(
            true);
      }
    });

    m_absolutePath = new BooleanFieldEditor2(
        TestNGPluginConstants.S_ABSOLUTEPATH, "Absolute output path", //$NON-NLS-1$ 
        SWT.NONE, parentComposite);
    m_outputdir.setAbsolutePathVerifier(m_absolutePath);

    m_disabledDefaultListeners = new BooleanFieldEditor2(
        TestNGPluginConstants.S_DISABLEDLISTENERS, "Disable default listeners", //$NON-NLS-1$ 
        SWT.NONE, parentComposite);

    // XML template
    m_xmlTemplateFile = new FileFieldEditor(
        TestNGPluginConstants.S_XML_TEMPLATE_FILE, "Template XML file:",
        false /* no absolute */,
        StringButtonFieldEditor.VALIDATE_ON_FOCUS_LOST, parentComposite);
    m_xmlTemplateFile.setEmptyStringAllowed(true);
    m_xmlTemplateFile.fillIntoGrid(parentComposite, 3);

    // Excluded stack traces
    m_excludedStackTraces = new StringFieldEditor(
    ...
    m_preDefinedListeners = new StringFieldEditor(
        TestNGPluginConstants.S_PRE_DEFINED_LISTENERS, "Pre Defined Listeners",
        parentComposite);
  ...
  private static class FSBrowseDirectoryFieldEditor extends
      DirectoryFieldEditor {
    BooleanFieldEditor2 absolutePath;

    public FSBrowseDirectoryFieldEditor(String name, String labelText,
        Composite parent) {
