...
public abstract class OpenEditorAction extends Action {
  ...
  public void run() {
    ...
    try {
      ...
      if (element == null) {
        MessageDialog.openError(getShell(), ResourceUtil
            .getString("OpenEditorAction.error.cannotopen.title"), //$NON-NLS-1$
            ResourceUtil.getFormattedString(
                "OpenEditorAction.error.cannotopen.message", fClassName)); //$NON-NLS-1$

        return;
      }
      textEditor = (ITextEditor) EditorUtility.openInEditor(element, fActivate);
    } catch (CoreException e) {
      ErrorDialog.openError(getShell(), ResourceUtil
          .getString("OpenEditorAction.error.dialog.title"), //$NON-NLS-1$
          ResourceUtil.getFormattedString(
              "OpenEditorAction.error.dialog.message", fClassName), //$NON-NLS-1$
          e.getStatus());

      return;
    }
    if (textEditor == null) {
      fTestRunner.setInfoMessage(ResourceUtil.getFormattedString(
          "OpenEditorAction.error.cannotopen.message", fClassName)); //$NON-NLS-1$
