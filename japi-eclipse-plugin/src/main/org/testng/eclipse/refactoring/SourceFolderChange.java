...
public class SourceFolderChange extends CompositeChange {
  ...
  private Change createChange(IResource resource) {
    ...
    // We need the binding to be able to retrieve the types of the expressions
    // in
    // the visitor (this is used to determine whether the assert() and fail()
    // method
    // invocations can be found on the AssertJUnit class or not. If not, we
    // don't
