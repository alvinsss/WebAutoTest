...
public abstract class TestRunTab {
   ...
   * @param clipboard
   *          the clipboard to be used by the tab
   * @param runner
   *          the testRunnerViewPart containing the tab folder
   */
  public abstract Composite createTabControl(Composite parent,
      TestRunnerViewPart runner);

  /**
   * @return the id of the currently selected node in the tree view or null if
   *         no nodes are selected. If the user selects an item on a tab and
   *         then switches to another tab, the new tab can then try to make a
   *         best effort to keep the same item selected.
   */
  public abstract String getSelectedTestId();

  /**
   * Ask this tab to select the item with the given id, if possible. Used when
   ...
   * 
   * @param expand
   *          true if the item should be expanded in the tree
   */
