...
public class ASTMethodDescriptor implements IMethodDescriptor {
  ...
  public ASTMethodDescriptor(MethodDeclaration methodDeclaration,
  ...
  public String getSignature() {
    ...
    for (int i = 0; i < paramVars.size(); i++) {
      buf.append(TypeSignature
          .getSignature(((SingleVariableDeclaration) paramVars.get(i))
              .getType()));
    ...
    buf.append(")").append(getReturnTypeSignature());
  ...
  /**
   * Returns <code>true</code> if this <code>MethodDescriptor</code> is the same
   * as the o argument.
   * 
   * @return <code>true</code> if this <code>MethodDescriptor</code> is the same
   *         as the o argument.
   */
