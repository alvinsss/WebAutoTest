...
public class SuiteListSelectionDialog extends SelectionDialog {
  ...
  protected Control createDialogArea(Composite parent) {
    ...
    folderNames = new Table(dialogArea, SWT.SINGLE | SWT.BORDER | SWT.V_SCROLL
        | SWT.H_SCROLL);
    data = new GridData(GridData.FILL_BOTH);
    data.widthHint = 300;
  ...
  private String getParentLabel(IResource resource) {
    ...
    if (parent.getType() == IResource.ROOT) {
      // Get readable name for workspace root ("Workspace"), without duplicating
      // language-specific string here.
      ...
      public void run() {
        ...
        for (int i = 1; i < descriptorsSize; i++) {
          ...
          if (nextResource.getType() == currentResource.getType()
            ...
            // If we are merging resources with the same name, into a single
            // descriptor,
            // then we must mark the descriptor unsorted so that we will sort
            // the folder
            // names.
